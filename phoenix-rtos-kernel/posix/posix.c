...
int posix_maxChildren(pid_t *whoMaxChildren){
	...
	*whoMaxChildren = 0;
	...
	//unlock descriptor tree
	proc_lockClear(&posix_common.lock);
	return maxChildren;
}

void posix_getProcHeightCount(int **pHeight, int **pCount){
	process_info_t *process;
	**pHeight = -1;
	**pCount = -1;
	...
	int Height = -1;
	int Count = -1;
	while (process != NULL){ //Loop Through Each Process
		//**procHeight = (int)process->process;
		
		int currHeight = 0;
		int currProcess = (int)process->process;
		int ctdn = 100;
		while (currProcess > 0 && ctdn > 0){
			currHeight++;
			...
			currProcess = (int)posix_getppid(currProcess);
		}
